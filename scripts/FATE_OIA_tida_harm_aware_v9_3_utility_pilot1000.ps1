...
$output = "F:\FATE_Drive_runs\tida_harm_aware_v9_3_utility_pilot1000"
...
& "E:\Anaconda\envs\sbw39\python.exe" -u -m fate_oia.engine.train_tida_oia `
  ...
  --max-calib-samples 835 --max-test-samples 885 --max-optimizer-updates 126 `
  --schedule-total-updates 126 `
  --train-owners "object_intent_action_utility,object_intent_reason_utility" `
  --eval-every-epochs 3 --run-kind smoke --skip-ema-eval --skip-expanded-eval --device cuda
$code = $LASTEXITCODE
