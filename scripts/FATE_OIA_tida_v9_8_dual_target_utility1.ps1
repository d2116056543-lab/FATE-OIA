...
$output = "G:\FATE_Drive_runs\tida_v9_8_dual_target_utility1"
if (Test-Path $output) {
...
& "E:\Anaconda\envs\sbw39\python.exe" -u -m fate_oia.engine.train_tida_oia `
  ...
  --checkpoint "G:\FATE_Drive_runs\tida_v9_5_primary5584_utility3\checkpoint_best_test_joint.pth" `
  ...
  --output-dir $output --epochs 1 --batch-size 4 --gradient-accumulation-steps 6 `
  --context-chunk-size 2 --num-workers 4 --max-samples 5584 `
  --max-calib-samples 779 --max-test-samples 885 --max-optimizer-updates 233 `
  --schedule-total-updates 233 `
