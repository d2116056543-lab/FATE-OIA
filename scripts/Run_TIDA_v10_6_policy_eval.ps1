$paramOutput = $null
if ($args.Count -ge 2 -and $args[0] -eq "-Output") { $paramOutput = $args[1] }
$ErrorActionPreference = "Continue"
$worktree = "E:\sbw\FATE_Drive\fate_oia_tida_relational_flow_v8_10k_worktree"
$output = if ($paramOutput) { $paramOutput } else { "G:\FATE_Drive_runs\tida_v10_6_proper_policy_eval1" }
...
& "E:\Anaconda\envs\sbw39\python.exe" -u -m fate_oia.engine.train_tida_oia `
  ...
  --checkpoint "G:\FATE_Drive_runs\tida_v10_4_boundary_aligned_lateral1\checkpoint_best_test_action_mf1.pth" `
  ...
  --context-chunk-size 2 --num-workers 6 `
  --max-samples 2048 --policy-use-train-core `
  --max-calib-samples 838 --max-audit-samples 1128 --max-test-samples 885 `
  --evaluation-only --run-kind smoke --skip-ema-eval --skip-expanded-eval --device cuda `
