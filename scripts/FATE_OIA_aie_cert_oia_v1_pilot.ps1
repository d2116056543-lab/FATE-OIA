...
$selected = (Get-Content "$review\AIE_CERT_RUNTIME_PROFILE.json" -Raw | ConvertFrom-Json).selected
if (-not $selected) { throw 'runtime profile has no selected candidate' }
& $python -u -m fate_oia.engine.train_aie_cert_oia `
  --config configs/fate_oia_train_360x640_aie_cert_oia_v1.yaml `
  --output-dir .background_runs\aie_cert_oia_v1_pilot --run-kind pilot --epochs $Epochs `
  --batch-size $selected.batch --gradient-accumulation-steps $selected.accum --num-workers $selected.workers `
  --max-train-samples $MaxTrainSamples --max-calib-samples $MaxCalibSamples `
  --max-audit-samples $MaxAuditSamples --max-test-samples $MaxTestSamples --device cuda
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
