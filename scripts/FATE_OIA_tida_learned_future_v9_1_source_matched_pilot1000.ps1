...
$output = "F:\FATE_Drive_runs\tida_learned_future_v9_1_source_matched_pilot1000"
...
& "E:\Anaconda\envs\sbw39\python.exe" -u -m fate_oia.engine.train_tida_oia `
  ...
  --context-chunk-size 2 --num-workers 0 --max-samples 1000 `
  --max-calib-samples 835 --max-test-samples 885 --max-optimizer-updates 84 `
  --schedule-total-updates 84 --train-owners "object_intent_action,object_intent_reason" `
  --eval-every-epochs 2 --run-kind smoke --skip-ema-eval --skip-expanded-eval --device cuda
$code = $LASTEXITCODE
