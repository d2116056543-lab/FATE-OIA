...
def write_required_smoke_artifacts(output_dir: str | Path, metrics: dict[str, Any], last_outputs: dict[str, Any], grad_stats: dict[str, Any], branch_metrics: dict[str, Any]) -> None:
    ...
    write_json(out / "visual_gate_stats.json", {"mean": last_outputs["visual_gate"].mean(), "min": last_outputs["visual_gate"].min(), "max": last_outputs["visual_gate"].max()})
    write_json(out / "diva_evidence_stats.json", {"evidence_confidence_mean": last_outputs["evidence_confidence"].mean(), "action_evidence_shape": list(last_outputs["action_evidence_tokens"].shape)})
    write_json(out / "action_set_usage_stats.json", {"prototype_vectors_used": True, "num_action_prototypes": 8})
    write_json(out / "factor_selection_stats.json", {"selected_indices_shape": list(last_outputs["selected_factor_indices"].shape), "selected_weight_mean": last_outputs["selected_factor_weights"].mean()})
    ...
    write_json(out / "reason_tail_stats.json", {"reason_gate_mean": last_outputs["reason_gate"].mean(), "tail_indices": [5, 6, 9, 12, 14]})
    ...
    append_jsonl(out / "visual_samples_epoch_0.jsonl", {"sample_index": 0, "has_factor": True, "has_action": True, "has_reason": True, "selected_factor_indices": last_outputs["selected_factor_indices"][0].detach().cpu().tolist()})
