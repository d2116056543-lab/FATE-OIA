...
def target_credit_sign_loss(credit_action: torch.Tensor, credit_reason: torch.Tensor, action_targets: torch.Tensor, reason_targets: torch.Tensor) -> torch.Tensor:
    pos_action = (action_targets.unsqueeze(1) > 0.5).expand_as(credit_action)
    pos_reason = (reason_targets.unsqueeze(1) > 0.5).expand_as(credit_reason)
    loss_a = F.softplus(-credit_action[pos_action]).mean() if bool(pos_action.any()) else credit_action.new_tensor(0.0)
    loss_r = F.softplus(-credit_reason[pos_reason]).mean() if bool(pos_reason.any()) else credit_reason.new_tensor(0.0)
...
def compute_tfc_losses(out: dict, action_targets: torch.Tensor, reason_targets: torch.Tensor, weights: dict) -> dict[str, torch.Tensor]:
    ...
    lc = target_credit_sign_loss(out["credit_action"], out["credit_reason"], action_targets, reason_targets)
