...
class LENSLossRegistry:
    ...
    def __call__(self, out: dict[str, Tensor], batch: dict[str, Tensor], responsibilities: dict[str, Tensor] | None = None) -> tuple[Tensor, dict[str, Tensor]]:
        ...
        raw: dict[str, Tensor] = {
            ...
            "reason_formal": reason_asl_loss(out["reason_logits_formal"], reason),
            "reason_latent_aux": reason_asl_loss(out["reason_logits_latent"], reason),
            "reason_rank": reason_rank_loss(out["reason_logits_formal"], reason),
            "reason_soft_f1": reason_soft_f1_loss(out["reason_logits_formal"], reason),
            "emission_prior": emission_prior_loss(out["emission_prob"]),
        ...
        if responsibilities is None:
            ...
            raw["emission"] = emission_loss(out["emission_prob"], reason, responsibilities["gamma"])
        structured = batch.get("structured")
        if structured is None:
            zero = out["evidence_map"].sum() * 0.0
            raw.update({"map_anchor": zero, "state_anchor": zero, "view_consistency": zero})
        else:
            ...
            raw["view_consistency"] = out["evidence_map"].sum() * 0.0
        total = sum(self.weights.get(name, 0.0) * value for name, value in raw.items())
        return total, raw
