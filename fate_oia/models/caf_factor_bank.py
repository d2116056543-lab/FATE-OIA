...
class CAFFactorBank(nn.Module):
    ...
    def _affinity_region_factors(self, dino_map: torch.Tensor) -> torch.Tensor:
    ...
    def forward(self, actor_evidence_tokens: torch.Tensor, dino_map: torch.Tensor, scene_state_proxy: torch.Tensor | None = None, train_mode: bool = False) -> dict[str, torch.Tensor]:
        ...
        object_factors = self._affinity_region_factors(dino_map)
        ...
        return {
            ...
            "factor_action_origin": torch.zeros(factor_tokens.shape[0], factor_tokens.shape[1], self.action_dim, device=factor_tokens.device),
        }
