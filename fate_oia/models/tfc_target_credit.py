...
class TFCTargetCredit(nn.Module):
    def __init__(self, num_factors: int, action_dim: int = 4, reason_dim: int = 21) -> None:
        super().__init__()
    ...
    def forward(
        ...
        learned_action = torch.tanh(self.learned_action).to(qrho.device, qrho.dtype)
        learned_reason = torch.tanh(self.learned_reason).to(qrho.device, qrho.dtype)
        ...
        credit_action = qrho.unsqueeze(-1) * (native_action + 0.25 * learned_action).unsqueeze(0) * action_gate.unsqueeze(1)
        credit_reason = qrho.unsqueeze(-1) * (native_reason + 0.25 * learned_reason).unsqueeze(0) * reason_gate.unsqueeze(1)
