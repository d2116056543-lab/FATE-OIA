...
class NativeTextReasonPairMemory(nn.Module):
    ...
    def enqueue(self, logits: torch.Tensor, targets: torch.Tensor, pu_state: dict) -> None:
        ...
        for r in range(self.reason_dim):
            ...
            for vals, buf, ptr, cnt in (
                (pos_vals, self.pos_logits, self.pos_ptr, self.pos_count),
                (neg_vals, self.neg_logits, self.neg_ptr, self.neg_count),
            ):
                if vals.numel() == 0:
                    continue
    ...
    def loss(
        ...
        if epoch < 7:
            ...
            return z, {
                "pair_count_total": 0,
                "tail_pair_count": 0,
                "zero_pair_count": int(logits.numel()),
                "memory_positive_coverage": int((self.pos_count > 0).sum().detach().cpu()),
                "memory_negative_coverage": int((self.neg_count > 0).sum().detach().cpu()),
                "cap_applied": False,
            }
        losses = []
        pair_count = 0
        tail_pair_count = 0
        hard_neg = pu_state["hard_negative_mask"] > 0.5
        for r in range(self.reason_dim):
            cur_pos = logits[targets[:, r] > 0.5, r]
            cur_neg = logits[hard_neg[:, r], r]
            mem_pos = self.pos_logits[r, : int(self.pos_count[r].item())].to(logits.device)
            mem_neg = self.neg_logits[r, : int(self.neg_count[r].item())].to(logits.device)
            pos_pool = torch.cat([cur_pos, mem_pos]) if mem_pos.numel() else cur_pos
            neg_pool = torch.cat([cur_neg, mem_neg]) if mem_neg.numel() else cur_neg
            if pos_pool.numel() == 0 or neg_pool.numel() == 0:
                continue
            pos_boundary = pos_pool[pos_pool.detach().abs() < near_boundary_delta]
            neg_boundary = neg_pool[neg_pool.detach().abs() < near_boundary_delta]
            if pos_boundary.numel() == 0:
                pos_boundary = pos_pool
            if neg_boundary.numel() == 0:
                neg_boundary = neg_pool
            margin = F.relu(0.2 - pos_boundary.mean() + neg_boundary.mean())
            losses.append(margin)
            pair_count += 1
            if r in self.tail_indices:
                tail_pair_count += 1
        ...
        return capped, {
            "pair_count_total": int(pair_count),
            "tail_pair_count": int(tail_pair_count),
            "zero_pair_count": int(self.reason_dim - pair_count),
            "memory_positive_coverage": int((self.pos_count > 0).sum().detach().cpu()),
            "memory_negative_coverage": int((self.neg_count > 0).sum().detach().cpu()),
            "cap_applied": bool(raw.detach().cpu() > cap.detach().cpu()),
        }
