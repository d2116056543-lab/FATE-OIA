...
class ReasonFromFactorDecoder(nn.Module):
    def __init__(self, hidden_dim: int = 256, reason_dim: int = 21, tail_indices: tuple[int, ...] = (12, 9, 5, 14, 6, 11, 10, 13)) -> None:
        super().__init__()
    ...
    def forward(self, selected_embeddings: torch.Tensor, scene_state_tokens: torch.Tensor) -> dict[str, torch.Tensor]:
        b = selected_embeddings.shape[0]
        memory = torch.cat([selected_embeddings.reshape(b, -1, selected_embeddings.shape[-1]), scene_state_tokens], 1)
        ...
        return {"reason_logits": logits, "reason_attention": attn, "reason_memory_tokens": memory}
