...
class MaskedReasonFromFactorTransformer(nn.Module):
    def __init__(self, dim: int = 384, action_dim: int = 4, reason_dim: int = 21, reason_cap: float = 0.25, num_heads: int = 4, tail_reason_indices: tuple[int, ...] = (5, 6, 9, 11, 12, 14)) -> None:
        super().__init__()
        self.action_dim = int(action_dim)
        self.reason_dim = int(reason_dim)
        self.reason_cap = float(reason_cap)
        self.tail_reason_indices = tuple(int(i) for i in tail_reason_indices)
        self.reason_queries = nn.Parameter(torch.randn(reason_dim, dim) * 0.02)
        self.tail_reason_queries = nn.Parameter(torch.randn(len(self.tail_reason_indices), dim) * 0.02)
        self.attn = nn.MultiheadAttention(dim, num_heads, batch_first=True)
        self.reason_head = nn.Linear(dim, 1)
        self.tail_head = nn.Linear(dim, 1)
    ...
    def forward(self, selected_factor_tokens: torch.Tensor, actor_evidence_tokens: torch.Tensor, base_reason_logits: torch.Tensor, scene_state_tokens: torch.Tensor | None = None) -> dict[str, torch.Tensor]:
        ...
        if len(self.tail_reason_indices) > 0:
            tq = self.tail_reason_queries.unsqueeze(0).expand(b, -1, -1)
            tout, tattn = self.attn(tq, context, context, need_weights=True, average_attn_weights=True)
            tail_delta = self.tail_head(tout).squeeze(-1)
            for pos, idx in enumerate(self.tail_reason_indices):
                if idx < self.reason_dim:
                    reason_factor[:, idx] = reason_factor[:, idx] + 0.25 * torch.tanh(tail_delta[:, pos])
        ...
        return {
            ...
            "tail_reason_indices": torch.tensor(self.tail_reason_indices, device=base_reason_logits.device, dtype=torch.long),
        }
