    ...
    def _iter_items(self, data: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for frame in data.get("frames", []):
            if isinstance(frame, dict):
                ...
                items.extend([x for x in frame.get("objects", []) if isinstance(x, dict)])
        items.extend([x for x in data.get("labels", []) if isinstance(x, dict)])
        items.extend([x for x in data.get("objects", []) if isinstance(x, dict)])
        return items
    ...
    def _record_targets(self, items: list[dict[str, Any]], drivable_available: bool = False) -> tuple[set[str], dict[str, int], dict[str, float]]:
        ...
        for item in items:
            ...
            if cat in {"traffic light"}:
                ...
                attr_text = json.dumps(item.get("attributes", {})).lower()
                if "green" in attr_text:
            ...
            if cat in {"traffic sign"}:
                ...
                attr_text = json.dumps(item.get("attributes", {})).lower()
                if "stop" in attr_text or "stop" in str(item.get("name", "")).lower():
                    names.add("stop_sign_present")
            if cat in {"car", "truck", "bus"}:
                if region == "left_corridor":
                    names.update(["vehicle_left"])
                    if "parked" in json.dumps(item.get("attributes", {})).lower():
                        names.add("parked_vehicle_left")
                elif region == "right_corridor":
                    names.update(["vehicle_right"])
                    if "parked" in json.dumps(item.get("attributes", {})).lower():
                        names.add("parked_vehicle_right")
                else:
                    _, y_norm = _norm_xy(0.5, (float((box or {}).get("y2", 0.0)) if isinstance(box, dict) else 0.0))
                    names.add("front_vehicle_close" if y_norm > 0.55 else "front_vehicle_far")
