...
class AIEOIAModel(nn.Module):
    ...
    def decode_from_field(
        ...
        *,
        ...
        reason = self.reason_private(
            ...
            evidence["evidence_token"],
            evidence["evidence_map"],
            contribution["bounded_contribution"],
            primary["predicate_attention"],
            primary["predicate_probs"],
    ...
    def forward(self, images: Tensor, *, action_scale: float = 1.0, reason_scale: float = 1.0) -> dict[str, Any]:
        return self.decode_from_field(
            self.encode_images(images), action_scale=action_scale, reason_scale=reason_scale
        )
