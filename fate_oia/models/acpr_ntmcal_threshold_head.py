...
class NativeTextMetricCalibrator(nn.Module):
    ...
    def forward(self, action_logits: torch.Tensor, reason_logits: torch.Tensor, support_score: torch.Tensor, contra_score: torch.Tensor, reason_rho: torch.Tensor, base_reason_logits: torch.Tensor, q_pred: torch.Tensor, rho_pred: torch.Tensor, epoch: int = 0) -> dict[str, torch.Tensor | dict]:
        ...
        return {"theta_action": theta_action, "theta_reason": theta_reason, "threshold_delta_reason": threshold_delta_reason, "threshold_delta_action": threshold_delta_action, "action_logits_deploy": action_logits - theta_action, "reason_logits_deploy": reason_logits - theta_reason, "logits_deploy": torch.cat([action_logits - theta_action, reason_logits - theta_reason], dim=-1), "threshold_stats": {"threshold_delta_reason_abs_mean": float(threshold_delta_reason.abs().mean().detach().cpu()), "threshold_delta_action_abs_mean": float(threshold_delta_action.abs().mean().detach().cpu())}}
