...
class ACPRNTMCalModel(nn.Module):
    def __init__(self, dim: int = 384, action_dim: int = 4, reason_dim: int = 21, selected_layers: tuple[int, ...] = (3, 7, 11), pretrained_weights: str = "ckp/reference/dino_deitsmall8_pretrain.pth", predicate_config: str = "configs/acpr_ntmcal_native_text_predicates.yaml", reason_formula_config: str = "configs/acpr_ntmcal_reason_formulas.yaml", use_mock_dino: bool = False, predicate_topk: int = 64) -> None:
        super().__init__()
        ...
        self.predicate_measurement = NativeTextTopKPredicateMeasurement(self.predicate_bank, self.atom_encoder, dim=dim, selected_layers=selected_layers, topk=predicate_topk)
    ...
    def forward(self, images: torch.Tensor, *, epoch: int = 0, split: str = "train", reason_labels: torch.Tensor | None = None, file_names=None, structured_records=None, force_zero_reason_delta: bool = False) -> dict:
        ...
        obs = self.observation_builder(reason_labels if self.training else None, file_names=file_names, structured_records=structured_records, split="train" if self.training else "test")
        pu = self.pu_builder(reason_labels if self.training else None, pred["predicate_q"], pred["predicate_rho"], epoch)
        reason_res = self.reason_residual(base_reason_logits, trunk["label_nodes"][:, self.action_dim:], pu["support_score"], pu["contra_score"], pu["reason_rho"], epoch=epoch)
        ...
        cal = self.ntmcal_threshold(action_logits, reason_logits, pu["support_score"], pu["contra_score"], pu["reason_rho"], base_reason_logits, pred["predicate_q"], pred["predicate_rho"], epoch=epoch)
        out = {**field, **trunk, **pred, **reason_res, **action_pred, **cal}
        out.update({"action_logits_base": base_action_logits, "reason_logits_base": base_reason_logits, "action_logits_ntmcal": action_logits, "reason_logits_ntmcal": reason_logits, "native_text_atoms": self.predicate_bank.audit(), "native_text_observations": obs, "pu_state": pu, "support_score": pu["support_score"], "contra_score": pu["contra_score"], "reason_reliability": pu["reason_reliability"], "ntmcal_stats": {"ego": ego_stats, "predicate": pred["predicate_stats"], "pu": pu["stats"], "reason_delta": reason_res["reason_delta_stats"], "action_predicate": action_pred["action_predicate_stats"], "threshold": cal["threshold_stats"]}, "branch_logits": {"base_fixed": torch.cat([base_action_logits, base_reason_logits], -1), "deploy_fixed": cal["logits_deploy"]}})
        return out
