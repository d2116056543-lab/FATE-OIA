    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            row.update({
                ...
                "action_rank_safety_rate": float(out["action_rank_safety_mask"].float().mean().detach().cpu()),
                "action_deploy_safety_rate": float(out["action_deploy_safety_mask"].float().mean().detach().cpu()),
                "traffic_control_upper_region_mass_action": float(out["artifact_stats"]["traffic_control_upper_region_mass_action"].detach().cpu()),
                "obstacle_front_center_mass_action": float(out["artifact_stats"]["obstacle_front_center_mass_action"].detach().cpu()),
                "left_lane_corridor_mass_action": float(out["artifact_stats"]["left_lane_corridor_mass_action"].detach().cpu()),
                "right_lane_corridor_mass_action": float(out["artifact_stats"]["right_lane_corridor_mass_action"].detach().cpu()),
            })
            last_loss_row = row
            last_train_stats = {
                ...
                "left_lane_corridor_mass_action": float(out["artifact_stats"]["left_lane_corridor_mass_action"].detach().cpu()),
                "right_lane_corridor_mass_action": float(out["artifact_stats"]["right_lane_corridor_mass_action"].detach().cpu()),
                "traffic_control_upper_region_mass_reason": float(out["artifact_stats"]["traffic_control_upper_region_mass_reason"].detach().cpu()),
                "obstacle_front_center_mass_reason": float(out["artifact_stats"]["obstacle_front_center_mass_reason"].detach().cpu()),
                "left_lane_corridor_mass_reason": float(out["artifact_stats"]["left_lane_corridor_mass_reason"].detach().cpu()),
                "right_lane_corridor_mass_reason": float(out["artifact_stats"]["right_lane_corridor_mass_reason"].detach().cpu()),
                "deletion_max_factors_used": float(out["artifact_stats"]["deletion_max_factors_used"].detach().cpu()),
                "same_region_background_is_ema": float(out["artifact_stats"]["same_region_background_is_ema"].detach().cpu()),
