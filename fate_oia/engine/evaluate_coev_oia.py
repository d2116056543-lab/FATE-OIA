...
def evaluate(model, loader, device: torch.device) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for batch_index, (inputs, targets) in enumerate(loader):
        out = model(inputs.to(device)); rows["logits"].append(out["logits"].cpu())
...
def final_intervention_diagnostics(model, loader, device: torch.device, limit: int = 256) -> dict[str, Any]:
    ...
    for inputs,targets in loader:
        if seen>=limit: break
        inputs=inputs.to(device); full=model(inputs); take=min(targets.action.shape[0],limit-seen)
    ...
    for name,row in views.items():
        ...
        if name=="full": full_pred=pred
            ...
            result[name]["versus_full_flips"]={
                ...
                "mean_abs_probability_delta":float(np.abs(score-np.concatenate(views["full"]["logits"]).numpy()).mean()),
            }
