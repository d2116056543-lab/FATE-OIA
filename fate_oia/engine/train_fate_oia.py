...
def write_epoch_artifacts(output_dir: Path, epoch: int, train_stats: dict[str, Any], val_stats: dict[str, Any], run_manifest: dict[str, Any]) -> None:
    ...
    torch.save(logits[:, run_manifest["config_resolved"].get("action_dim", 4):] if logits.numel() else torch.empty(0, 0), epoch_dir / "logits_reason.pt")
    action_dim = int(run_manifest["config_resolved"].get("action_dim", 4))
