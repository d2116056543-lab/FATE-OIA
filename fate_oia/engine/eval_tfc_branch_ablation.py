...
from fate_oia.engine.train_acpr_tfc_oia import build_model, make_loader, oracle_threshold_metrics
...
def summarize(logits: dict[str, torch.Tensor], action_labels: torch.Tensor, reason_labels: torch.Tensor) -> dict[str, Any]:
    summary: dict[str, Any] = {}
    for key in ["action_visual_only", "action_tfc_delta_off", "action_tfc_delta_on", "action_final_deploy"]:
        summary[key] = multilabel_metrics_from_logits(logits[key], action_labels, prefix="Act_")
    for key in ["reason_visual_only", "reason_tfc_delta_off", "reason_tfc_delta_on", "reason_final_deploy"]:
...
def main() -> None:
    ...
    summary = summarize(logits, action_labels, reason_labels)
