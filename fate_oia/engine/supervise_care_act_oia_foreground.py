...
def main() -> None:
    ...
    args = ap.parse_args()
    attempts = [(args.batch_size, args.gradient_accumulation_steps), (args.fallback_batch_size_1, args.fallback_grad_accum_1), (args.fallback_batch_size_2, args.fallback_grad_accum_2)]
    out_dir = args.output_dir or str(Path(".background_runs") / f"care_act_oia_v1_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    last_code = 1
    for bs, ga in attempts:
        cmd = [
            sys.executable, "-m", "fate_oia.engine.train_care_act_oia",
            "--config", args.config,
            ...
            "--bdd_oia_root", args.bdd_oia_root,
            "--data_root", args.bdd_oia_root,
            "--output_dir", out_dir,
            "--require_review_pass",
        ]
