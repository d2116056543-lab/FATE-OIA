...
def test_heca_factor_schema_is_complete_without_hard_action_masks() -> None:
    rows = yaml.safe_load(Path("configs/meter_factor_schema.yaml").read_text())["factors"]
    required = {
        "id", "name", "factor_type", "factor_group", "text_prompt", "state_set",
        "state_prompts", "anchor_source", "state_source", "groundability",
        "action_owned", "observability_source", "mirror_partner", "counter_localizable",
    }
