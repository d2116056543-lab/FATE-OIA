...
def test_progress_zero_preserves_foundation_outputs() -> None:
    model = METEROIAModel(use_mock_dino=True)
    out = model(torch.randn(1, 3, 360, 640), progress=0)
    torch.testing.assert_close(out["action_logits_final"], out["action_logits_calalign"], atol=1e-6, rtol=0)
