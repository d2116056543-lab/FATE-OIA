...
from fate_oia.losses.tida_losses import action_smooth_ap_loss, reason_rank_loss
...
def test_reason_rank_uses_detached_reference_from_same_accumulation_window():
    current = torch.tensor([[0.0]], requires_grad=True)
    target = torch.tensor([[1.0]])
    reference = torch.tensor([[0.5]], requires_grad=True)
    reference_target = torch.tensor([[0.0]])
    ...
    loss = reason_rank_loss(
        current,
        target,
        reference_logits=reference,
        reference_target=reference_target,
        reference_negative_weight=reference_weight,
    )
    loss.backward()
