...
def test_observed_reason_ranking_uses_only_known_positive_negative_pairs() -> None:
    """Synthetic-hidden entries must not become observed-reason negatives."""
    queue = MOSAICSoftRankQueue(label_dim=1, capacity=8)
    queue.enqueue(
        torch.tensor([[-1.0], [3.0]]),
        torch.tensor([[0.0], [float("nan")]]),
        ["known-negative", "unknown-history"],
    )
    current_logits = torch.tensor([[1.0]], requires_grad=True)
    loss, stats = observed_reason_cross_image_ranking_loss(
        current_logits,
        torch.tensor([[1.0]]),
        torch.tensor([[True]]),
        ["known-positive"],
...
def test_observed_reason_ranking_skips_hidden_current_positive() -> None:
    queue = MOSAICSoftRankQueue(label_dim=1, capacity=8)
    queue.enqueue(torch.tensor([[-1.0]]), torch.tensor([[0.0]]), ["history"])
